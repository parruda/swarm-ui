...
export default class extends Controller {
    ...
    this.svg.innerHTML = `
      <defs>
        <marker id="arrow" markerWidth="10" markerHeight="10" refX="9" refY="5" orient="auto" markerUnits="userSpaceOnUse">
          <path d="M 0 1 L 0 9 L 9 5 Z" fill="#f97316" stroke="none" />
        </marker>
        <marker id="arrow-selected" markerWidth="10" markerHeight="10" refX="9" refY="5" orient="auto" markerUnits="userSpaceOnUse">
          <path d="M 0 1 L 0 9 L 9 5 Z" fill="#ea580c" stroke="none" />
        </marker>
  ...
  handleConnectionDragEnd(e) {
    ...
    if (targetNode) {
      ...
      if (fromId !== targetNodeId) {
        ...
        // Find the best socket combination based on shortest path
        const fromNode = this.nodes.get(fromId)
        const toNode = this.nodes.get(targetNodeId)
        
        if (fromNode && toNode) {
          // Use intelligent socket selection
          const { toSide } = this.findBestSocketPairForDrag(fromNode, toNode, fromSide)
          ...
          if (targetSocket) {
            this.createConnection(fromId, fromSide, targetNodeId, toSide)
          }
        }
      }
    }
  ...
  findBestSocketPair(fromNode, toNode) {
    const fromX = fromNode.data.x + this.canvasCenter
    const fromY = fromNode.data.y + this.canvasCenter
    const toX = toNode.data.x + this.canvasCenter
    const toY = toNode.data.y + this.canvasCenter
    ...
    // Natural flow priority: right->left for horizontal, bottom->top for vertical
    if (Math.abs(dx) > Math.abs(dy) * 1.5) {
      ...
      // Diagonal - choose based on angle
    ...
    if (fromNode.data.id === this.mainNodeId && (fromSide === 'top' || fromSide === 'left')) {
      if (fromSide === 'top') fromSide = 'bottom'
      if (fromSide === 'left') fromSide = 'right'
    }
    
    // Check if target socket is used
  ...
  getAlternativeSockets(preferredSide) {
    const alternatives = {
    ...
    return alternatives[preferredSide] || []
  }
  
  // Create automatic connection between nodes
  createAutoConnection(fromId, toId) {
    const fromNode = this.nodes.get(fromId)
    const toNode = this.nodes.get(toId)
    
    if (!fromNode || !toNode) return
    
    // Find best socket pair
    const { fromSide, toSide } = this.findBestSocketPair(fromNode, toNode)
    
    // Create the connection
    this.createConnection(fromId, fromSide, toId, toSide)
  }
  ...
  updateConnections() {
    ...
    this.connections.forEach((conn, index) => {
      ...
      if (fromNode && toNode) {
        // Create professional path with intelligent routing
        const path = this.createConnectionPath(conn, fromNode, toNode, index)
        if (path) {
  ...
  createConnectionPath(conn, fromNode, toNode, index) {
    ...
    const toX = (toRect.left - viewportRect.left + toRect.width/2) / this.zoomLevel
    const toY = (toRect.top - viewportRect.top + toRect.height/2) / this.zoomLevel
    
    // Generate intelligent path
  ...
  generateSmartPath(x1, y1, fromSide, x2, y2, toSide) {
    // Generate smooth bezier curves like Figma
    const distance = Math.sqrt(Math.pow(x2 - x1, 2) + Math.pow(y2 - y1, 2))
    const minControlOffset = 50
    const maxControlOffset = 200
    const controlOffset = Math.min(maxControlOffset, Math.max(minControlOffset, distance * 0.5))
    ...
    // Special handling for opposite sides (straight connections)
    if ((fromSide === 'right' && toSide === 'left' && x2 > x1) ||
        (fromSide === 'left' && toSide === 'right' && x1 > x2) ||
        (fromSide === 'bottom' && toSide === 'top' && y2 > y1) ||
        (fromSide === 'top' && toSide === 'bottom' && y1 > y2)) {
      
      const straightOffset = 30
      if (fromSide === 'right' && toSide === 'left') {
        const midX = (x1 + x2) / 2
        cp1x = Math.max(x1 + straightOffset, midX)
        cp2x = Math.min(x2 - straightOffset, midX)
      } else if (fromSide === 'left' && toSide === 'right') {
        const midX = (x1 + x2) / 2
        cp1x = Math.min(x1 - straightOffset, midX)
        cp2x = Math.max(x2 + straightOffset, midX)
      } else if (fromSide === 'bottom' && toSide === 'top') {
        const midY = (y1 + y2) / 2
        cp1y = Math.max(y1 + straightOffset, midY)
        cp2y = Math.min(y2 - straightOffset, midY)
      } else if (fromSide === 'top' && toSide === 'bottom') {
        const midY = (y1 + y2) / 2
        cp1y = Math.min(y1 - straightOffset, midY)
        cp2y = Math.max(y2 + straightOffset, midY)
      }
    }
  ...
  autoLayout() {
    ...
    // Find the main node or most connected node
    const rootNodeId = this.mainNodeId || this.findMostConnectedNode()
    
    // Perform hierarchical layout
    const layout = this.calculateHierarchicalLayout(graph, rootNodeId)
    
    // Apply smooth transition
    this.applyLayoutWithAnimation(layout)
  }
  ...
  findMostConnectedNode() {
    ...
    this.nodes.forEach((node, id) => {
      const connections = this.connections.filter(c => c.from === id || c.to === id).length
    ...
    return mostConnected || this.nodes.keys().next().value
  }
  
  calculateHierarchicalLayout(graph, rootId) {
    const layout = new Map()
    const levelNodes = new Map() // level -> [nodeIds]
    ...
    while (queue.length > 0) {
      ...
      // Also check incoming for bidirectional relationships
    ...
    levelNodes.forEach((nodeIds, level) => {
      ...
      // Sort nodes by connections to minimize crossings
      const sortedNodes = this.sortNodesByConnections(nodeIds, graph, level > 0 ? levelNodes.get(level - 1) : [])
      
      sortedNodes.forEach((nodeId, index) => {
        layout.set(nodeId, {
          x: startX + index * horizontalSpacing,
          y: level * verticalSpacing - (levelNodes.size - 1) * verticalSpacing / 2
        })
      })
    })
  ...
  sortNodesByConnections(nodeIds, graph, previousLevelNodes) {
    if (previousLevelNodes.length === 0) return nodeIds
    ...
    const barycenters = nodeIds.map(nodeId => {
      ...
      node.incoming.forEach(sourceId => {
        const sourceIndex = previousLevelNodes.indexOf(sourceId)
        if (sourceIndex >= 0) {
          sum += sourceIndex
          count++
        }
  ...
  applyLayoutWithAnimation(layout) {
    // Add transition class to nodes
    this.nodes.forEach(node => {
    ...
    // Update connections with delay for smooth animation
    setTimeout(() => {
      this.updateConnections()
  ...
  handleImportFile(event) {
    ...
    reader.onload = (e) => {
      try {
          ...
          if (instance.connections && Array.isArray(instance.connections)) {
            instance.connections.forEach(targetKey => {
              ...
              if (toNodeId !== undefined) {
                // Use intelligent socket selection
                this.createAutoConnection(fromNodeId, toNodeId)
              }
            })
          }
        })
        ...
        // Auto-layout after import for optimal presentation
        setTimeout(() => {
          this.autoLayout()
        }, 100)
